...
const AppContext = (props) => {
  ...
  const addItem = async (id)=>{
    try{
      ...
      if(addedProduct && !addedProductIndex){  
        ...
        const response= await axios.post(`https://crudcrud.com/api/0da8b18f966f4518ba06563b1ce9ee2e/cart${userMail}`, addedProduct)
  ...
  const removeItem = async (itemid, id)=>{
    try{
      ...
      const userMail= localStorage.getItem('email');
      await axios.delete(`https://crudcrud.com/api/0da8b18f966f4518ba06563b1ce9ee2e/cart${userMail}/${id}`)
   } catch(err){
      console.log("Item not removed- " + err);
        ...
        const fetchItems= async()=>{
          try{
            const userMail= localStorage.getItem('email');
           const response= await axios.get(`https://crudcrud.com/api/0da8b18f966f4518ba06563b1ce9ee2e/cart${userMail}`);
